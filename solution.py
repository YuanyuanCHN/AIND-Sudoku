...
def naked_twins(values):
    ...
    for box in two_digit_boxes:
        for peer in peers[box]:
            if values[peer] == values[box]:
                if [peer,box] not in naked_twins:
    ...
    for unit in unitlist:
        for i in range(len(naked_twins)):
            if (naked_twins[i][0] in unit) & (naked_twins[i][1] in unit):
                for box in unit:
                    if box not in naked_twins[i]:
                        digits = values[naked_twins[i][0]]
                        if len(digits) < 2:
                            return(values)
...
diag_unit = [[rows[i]+cols[i] for i in range(9)],[rows[i]+cols[8-i] for i in range(9)]]
